...
private struct BalanceGraphModelTests {
    @Test(arguments: [
        BalanceGraphModel(
            id: "test-id",
            yearMonth: "25/03",
    ...
    func testInitWithParameters(_ model: BalanceGraphModel) {
        #expect(model.id == "test-id")
        #expect(model.yearMonth == "25/03")
        #expect(model.amount == 100000)
    }

    struct InitFromBalanceTestCase {
        let year: Int
        let month: Int
    ...
    @Test(arguments: [
        InitFromBalanceTestCase(year: 2025, month: 3, expectedYearMonth: "25/03"),
        InitFromBalanceTestCase(year: 2024, month: 12, expectedYearMonth: "24/12"),
        InitFromBalanceTestCase(year: 2025, month: 1, expectedYearMonth: "25/01"),
        InitFromBalanceTestCase(year: 2000, month: 6, expectedYearMonth: "00/06"),
        InitFromBalanceTestCase(year: 2099, month: 7, expectedYearMonth: "99/07")
    ])
    func testInitFromBalance(testCase: InitFromBalanceTestCase) {
        let balance = Balance(
            id: "balance-id",
            account: "現金",
            year: testCase.year,
        ...
        let model = BalanceGraphModel(balance)

        #expect(model.id == "balance-id")
    ...
    @Test
    func testEquatable() {
        let model1 = BalanceGraphModel(
            id: "same-id",
            yearMonth: "25/03",
            amount: 100000
        )

        let model2 = BalanceGraphModel(
            id: "same-id",
            yearMonth: "25/03",
            amount: 100000
        )

        let model3 = BalanceGraphModel(
            id: "different-id",
            yearMonth: "25/03",
            amount: 100000
        )
